...
class Parser {
  constructor(str, filepath) {
    ...
    const _push = [].push // eslint-disable-line
    deps.push = function push() {
      [].slice.call(arguments, 0).forEach(item => { // eslint-disable-line
        if (deps.indexOf(item) < 0) {
          _push.call(deps, item)
        }
      })
    }
    this.childTpl = {}
    const parser = new htmlparser.Parser({
      onopentag: (name, attribs) => {
        ...
        if (is(name)) {
          if (fs.existsSync(path.join(filepath, `${name}.art`))) {
            ...
            this.write(`_${fnName}_tpl.call(this, ${transformer.attrs(attribs)})`)
            return
          }
  ...
  createChildFn(name, parser) {
    ...
    this.insert(`function _${name}_tpl(props) {
${res.output}
}`)
  }
