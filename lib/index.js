      ...
      onopentag: (name, attribs) => {
        ...
        if (is(name)) {
          ...
          if (fs.existsSync(filename)) {
            ...
            this.write(`_${fnName}_tpl.call(this, ${transformer.attrs(attribs)}, function (props) {`) // eslint-disable-line
            this.indent++
            return
          }
...
class ChildParser extends Parser {
  output() {
    const start = this.buffer.shift()
    const end = this.buffer.join('\n')
    const dependencies = this.deps.slice()
    const resource = this.resource.slice()
    return { start, end, dependencies, resource }
  }
}
