...
const reactAttrMap = {
  'class': 'className', // eslint-disable-line
  colspan: 'colSpan',
  rowspan: 'rowSpan',
  autoplay: 'autoPlay',
}
...
const attrs = (obj) => {
  ...
  attrList
    .forEach(key => {
      if (reactAttrMap[key]) { // fix all html attribute for react
        res += `  ${reactAttrMap[key]}: ${attr(obj[key])},\n`
      } else {
...
const tokens = [
  ...
  [/^\/if/, 'if-end'],
  [/^else( +if +([\s\S]+))?/, 'else'],
  [/^each ([\s\S]+) as ([a-zA-Z0-9]+)( ([a-zA-Z0-9]+))?/, 'each-start'],
  [/^\/each/, 'each-end'],
  [/^include (['"])([^'"]+)\1/, 'include'],
]
  ...
  if (res) {
    switch (res.type) {
      ...
      case 'each-start': // eslint-disable-line
        const indexKey = res.data[3] ? res.data[4] : genI()
        const l = genL()
        output.push({
          needKey: indexKey,
          aIndent: 1,
      ...
      case 'each-end':
        ...
        break
      case 'include':
        throw new Error('Didn\'t support include!')
      default:
